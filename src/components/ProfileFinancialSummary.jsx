...
export const ProfileFinancialSummary = ({
  ...
  return (
    <div className="profile-summary card-soft">
      ...
      {/* Income vs Spending — new flow bar */}
      {(monthlyEarned > 0 || monthlySpent > 0) && (
        <div className="flow-section" aria-label="Income versus spending this month">
          <div className="flow-header">
            <span className="flow-label">Income vs Spending</span>
            <span
              className={`flow-net ${
                netFlow >= 0 ? "flow-net--positive" : "flow-net--negative"
              }`}
            >
              {netFlow >= 0 ? "+" : "-"}
              {formatMoney(Math.abs(netFlow))} net
            </span>
          </div>
          <div
            className="flow-bar"
            role="img"
            aria-label={`${Math.round(earnedPct)}% earned, ${Math.round(spentPct)}% spent`}
          >
            {monthlyEarned > 0 && (
              <div
                className="flow-bar__segment flow-bar__segment--earned"
                style={{ width: `${earnedPct}%` }}
                title={`Earned: ${formatMoney(monthlyEarned)}`}
              />
            )}
            {monthlySpent > 0 && (
              <div
                className="flow-bar__segment flow-bar__segment--spent"
                style={{ width: `${spentPct}%` }}
                title={`Spent: ${formatMoney(monthlySpent)}`}
              />
            )}
          </div>
          <div className="flow-legend">
            <div className="flow-legend-item">
              <span
                className="flow-legend-dot flow-legend-dot--earned"
                aria-hidden="true"
              />
              <span className="flow-legend-text">
                <strong>{formatMoney(monthlyEarned)}</strong> earned
              </span>
            </div>
            <div className="flow-legend-item">
              <span
                className="flow-legend-dot flow-legend-dot--spent"
                aria-hidden="true"
              />
              <span className="flow-legend-text">
                <strong>{formatMoney(monthlySpent)}</strong> spent
              </span>
            </div>
          </div>
        </div>
      ...
      {totalTransactions === 0 && (
        <p className="summary-empty">
          Start adding transactions to see your financial insights here.
        </p>
      )}
    </div>
  );
